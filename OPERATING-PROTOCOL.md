...
## Discovery, usefulness and efficiency — 2026-09-23

The user requested a measurable standard for maximum efficiency, discovery and use case. Read `memory/discovery-efficiency-standard.json` and its human twin `DISCOVERY-EFFICIENCY-STANDARD-2026-09-23.md` for public liquidity work. Preserve exact asset identities, distinguish provider coverage from exhaustive ledger coverage, keep public inspection ungated, explain measured liquidity limitations, and load bounded pages (50 initially; at most 100 rendered) with explicit continuation. Diagnose errors by cause and use permitted public data paths. Run only focused checks that resolve changed risks, record actual completion evidence and stop. These requirements do not claim a completed application repair and do not clear unrelated queued issues.
